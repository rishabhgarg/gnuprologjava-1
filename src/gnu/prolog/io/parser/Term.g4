...
//Have to catch and throw the parse exception
...
locals [boolean pIs1201, boolean pIsZero, int p]
@init
{   $p = 0;
...
@init
{
    ...
    CompoundTermTag f = null;
        ...
        // Only the above line needs to be uncommented when mutual left recursion resolved  
        (
            ...
            (
                ...
                |
                (
                {$token_name == INTEGER_TOKEN}? {$token.getText().charAt(0) == '-'}?
                INTEGER_TOKEN
                {
                    $t = createTerm(CompoundTermTag.minus2,$t,IntegerTerm.get(-(IntegerTerm.get($token.getText()).value)));
                }
                )+
                |
                (
                {$token_name == FLOAT_NUMBER_TOKEN}? {$token.getText().charAt(0) == '-'}?
                FLOAT_NUMBER_TOKEN
                {
                    $t = createTerm(CompoundTermTag.minus2,$t,new FloatTerm(-(new FloatTerm($token.getText()).value)));
                }
                )+
                |
                (
                {testYFX($Options, $priority)}?
                m=op2[$Options, p]      {f = $m.tag;}
                n=term[$Options, p-1]   {t2 = $n.t; $t = createTerm(f, $t, t2);}
                )+
                |
                (
                {testYF($Options, $priority)}?
                o=op2[$Options, p]      {$t = createTerm(f, $t);}
                )+
            )
        )      
        ; 
...
@init
{
    ...
    : {$priority >= 1000}? {getCurrentToken().getType() == COMMA_TOKEN}? 
      comma {$tag = TermConstants.conjunctionTag;}  
    | NAME_TOKEN  
    {
        Token tk = $NAME_TOKEN; 
        NameToken ntk = new NameToken(tk);
...
@init
{
    ...
    : {_input.LA(2) == OPEN_TOKEN}?
      a=compound[$Options]  {$t = $a.rc;}
    | {testNoOp($Options)}? 
      b=name                {$t = $b.alpha;} 
    | c=variable[$Options]  {$t = $c.var;}
    | d=integer             {$t = $d.int_term;} 
    | e=float_number        {$t = $e.float_term;}
    | f=list_term[$Options] {$t = $f.rc;}
    | g=curly_term[$Options]{$t = $g.curled_term;}
    | h=char_code_list      {$t = $h.char_term;}
    | open
      i=term[$Options, 1201]{$t = $i.t;}
      close
    ;

//list_term might have errors in returning values
...
@init
{
    ...
    ( 
        ...
        ht_sep
        c=exp[$Options] {((CompoundTerm)t1).args[1] = t2;}
    )?
      close_list
    {
        if ($rc == null)
